...
## Hypotese:
Training affects Total RNA more in HIGH responders than in LOW responders after a training periode.
0 hypoteses: no change inbetween groups
...
## Making data set and selecting variables. The T3T1_PERCENT_CHANGE_RNA variable is the percentage change in total RNA content per muscle weight from the beginning to the end of the training period, SUB_ID is subjects, cluster differentiating between high and low responders.
...
hyp2 <- hypertrophy1 %>%
  ...
  print()
```
# The data are paired, meaning that we now have two data points from each participant. In the t-test function we need to specify paired = TRUE in this case. The null hypothesis is to be directional or “two.sided”.
...
hyp3 <- hypertrophy1 %>%
  ...
  print()
```
## The t-test is next to be used as formula. We are in the next section explaining the variable T3T1__RNA with the grouping variable (CLUSTER). In R we can write this as a formula T3T1__RNA  ~ CLUSTER. 

```{r}
t.test(T3T1__RNA  ~ CLUSTER, data = hyp3)
```

#The mean increase in the LOW group is a bit higher than in the HIGH group.
...
##Conclusion: The mean increase in the LOW group is a bit higher than in the HIGH group, however the results are not so different than what would be expected in the null hypothesis was true. We do not have strong evidence against the null
